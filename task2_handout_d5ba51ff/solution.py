...
class SGLDTrainer(Framework):
    ...
    def predict_probabilities(self, x: torch.Tensor) -> torch.Tensor:
        ...
        estimated_probability = torch.zeros(x.shape[0],10)
        for mod in self.SGLDSequence:
            estimated_probability += F.softmax(mod(x), dim=1)

        estimated_probability = estimated_probability / len(self.SGLDSequence)
...
class BayesNet(nn.Module):
    ...
    def predict_probabilities(self, x: torch.Tensor, num_mc_samples: int = 50) -> torch.Tensor:
        ...
        :param num_mc_samples: Number of MC samples to take for prediction
        :return: Predicted class probabilities, float tensor of shape (batch_size, 10)
            such thFt the last dimension sums up to 1 for each row
        """
...
def main():
    ...
    raw_train_data = np.load("/home/alex/git/PAI_Projects/task2_handout_d5ba51ff/train_data.npz")
